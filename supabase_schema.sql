...
--    (reglas de seguridad abiertas para uso interno).
--
-- ⚠️  SEGURIDAD (v42):
...
-- Supabase). El `id` es numérico (Date.now() en la app) y primary key.
-- Esto permite updates/upserts atómicos por id sin tocar campos
-- individuales sobre Postgres.
...
-- Patrón singleton: una fila por nombre, con data JSONB.
...
--  ROW LEVEL SECURITY
-- ══════════════════════════════════════════════════════════════════════
-- IMPORTANTE: la app NO usa Supabase Auth (contraseñas propias
...
-- de admin". Este es el modelo de seguridad usado desde siempre
-- (validación en el lado de la app, no en la base de datos).
...
-- Acceso público de lectura/escritura (RLS deshabilitado o permisivo).
